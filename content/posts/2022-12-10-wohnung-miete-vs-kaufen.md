...
入居時に用意しておくべき資金 $F_0$ は下記のようになる。

<div>
$$
\begin{aligned}
F_0 &= P(0) + \sum_{i = 0}^{N - 1} c (1+r)^{-i} \\
 &= P(0) + c\frac{1}{r} \left\{1 - (1+r)^{-N+1}\right\} \\
\end{aligned}
$$
</div>
...
<div>
$$
R = \frac{r}{1+r}\frac{\left\{(1+r)^N (F_0 - c_0) - F(N) \right\}}{(1+r)^{N} - 1}
$$
</div>
...
一方で物件を賃貸して $N$ 年後に退去した場合、
退去時点での手持ち資金は $F(N)$ である。

最初に資金 $F_0$ を用意し、
$N$ 年後に同額の手持ち資金が残れば物件の購入・賃貸は経済的に得失なしといえる。
この場合の家賃を均衡家賃 $R_{eq}$ と呼ぶことにすると、
$R_{eq}$ は上で求めた家賃 $R$ で $P(N) = F(N)$ となる場合に相当する。
<div>
$$
\begin{aligned}
F_0 &= P(0) + c\frac{1}{r} \left\{1 - (1+r)^{-N+1}\right\}  \\
R_{eq} &=
    \frac{r}{(1+r)}
    \frac{1}{(1+r)^{N} - 1}
...
入居時必要費用 $F_0$ は約４６３０万円となる。

購入後は年間２．５％の割合でマンションの価格が下がっていくとすると、
１５年後のマンションの価値は約２４６０万円となる。
物件売却によって手にする金額 $P(N)|_{N=15}$ は、
さらにこれから仲介手数料を差し引いて約２３８０万円となる。

前節の式に代入すると均衡家賃 $R_{eq}$ は約２８０万円／年。
２年に１度更新手数料などが発生することを想定すると、
入居時手数料４５万円、家賃＋管理費が月２２万５千円程度であれば、
１５年後の退去時に売却価格と同額の２千２８０万円の資産が手元に残ることになる。

なお運用利率 $r$ を保守的に２％程度と見積もると、
均衡家賃 $R_{eq}$ は月１７万５千円程度に下がる。

単純に購入時のマンション価格＋諸費用 $P(0)$ と売却価格 $P(N)|_{N=15}$ の差を１５年間で按分すると８万２千円／月となるが、
...
５年しか入居せずに売却すると２割程度負担が重くなり、
逆に２５年入居した場合には８％程度負担が軽くなる。
...
売却時の不動産市況により物件売却価格は上下する可能性があるが、
物件価格が上昇した場合には想定よりも多額の金額が手元に残ることになる。
一方でマンションを購入した場合、
地震などにより不動産価値が毀損するリスクもある。

居住地としてマンションを購入することには、
値上がりに賭けて不動産投資を行っているという側面もある。
